...
class MockGame:
    ...
    def __init__(self):
        ...
        self.dice_results = {'white1': 3, 'white2': 4, 'red': 2, 'yellow': 5, 'green': 1, 'blue': 6}
...
def test_ai_evaluation():
    ...
    for color, number in test_cases:
        ...
        print(f"{color.value}\t{number}\t{easy_score:.1f}\t{medium_score:.1f}\t{hard_score:.1f}")
    ...
    for difficulty in ["easy", "medium", "hard"]:
        ...
        print(f"{difficulty.capitalize()} AI - Stage 1 move rate: {stage_1_rate:.2f}, Stage 2 move rate: {stage_2_rate:.2f}")
...
def test_red_11_penalty():
    ...
    assert score_red_11_empty < score_red_11_with_marks, "Red 11 should have lower score when red row is empty"
    ...
    assert penalty_difference >= 10, f"Penalty should be at least 10 points, got {penalty_difference:.1f}"
...
def test_stage_1_move_probability():
    ...
    for difficulty in ["easy", "medium", "hard"]:
        ...
        expected_min_rates = {
            "easy": 0.6,    # Should make moves at least 60% of the time
            "medium": 0.7,  # Should make moves at least 70% of the time
            "hard": 0.75    # Should make moves at least 75% of the time
        }
        
        min_rate = expected_min_rates[difficulty]
        assert move_rate >= min_rate, f"{difficulty} AI should make Stage 1 moves at least {min_rate*100}% of the time, got {move_rate*100:.1f}%"

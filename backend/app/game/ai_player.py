...
class AIPlayer(Player):
    ...
    def make_move_decision(self, game, available_moves: List[Tuple[DieColor, int]]) -> Optional[Tuple[DieColor, int]]:
        """
        ...
        self.logger.debug(f"{self.name} considering {len(available_moves)} moves: {available_moves}")
        ...
        if decision:
            ...
            self.logger.info(f"{self.name} decided to mark {number} in {color.value} row")
            log_game_event(
                "AI_DECISION",
    ...
    def _make_easy_decision(self, available_moves: List[Tuple[DieColor, int]]) -> Optional[Tuple[DieColor, int]]:
        """Easy AI: Random selection with 70% chance to make a move."""
        if random.random() < 0.3:  # 30% chance to skip
            return None
        return random.choice(available_moves)
    
    def _make_medium_decision(self, game, available_moves: List[Tuple[DieColor, int]]) -> Optional[Tuple[DieColor, int]]:
    ...
    def _make_hard_decision(self, game, available_moves: List[Tuple[DieColor, int]]) -> Optional[Tuple[DieColor, int]]:
        ...
        if random.random() < 0.95:
            ...
            return (scored_moves[0][1], scored_moves[0][2]) if len(scored_moves) == 1 else (scored_moves[1][1], scored_moves[1][2])
    ...
    def _evaluate_move(self, game, color: DieColor, number: int) -> float:
        ...
        early_game_penalty = self._calculate_early_game_positioning_penalty(color, number, marked_count)
    ...
    def _evaluate_move_advanced(self, game, color: DieColor, number: int) -> float:
        ...
        opponent_blocking_bonus = self._calculate_opponent_blocking_bonus(game, color, number)
        ...
        if locked_colors >= 1:  # End game approaching
            ...
            potential_score = self._calculate_potential_row_score(row, len(row.marked) + 1)
            score += potential_score * 0.7  # Increased weight
        
        # 7. Advanced early game positioning analysis
        advanced_positioning_bonus = self._calculate_advanced_positioning_bonus(game, color, number)
    ...
    def _calculate_opponent_blocking_bonus(self, game, color: DieColor, number: int) -> float:
        """
    ...
    def _calculate_enhanced_row_lock_value(self, game, color: DieColor, number: int) -> float:
        """
    ...
    def _calculate_early_game_positioning_penalty(self, color: DieColor, number: int, marked_count: int) -> float:
        """
    ...
    def _calculate_red_11_penalty(self, color: DieColor, number: int) -> float:
        ...
        if len(red_row.marked) == 0:
            return -5.0  # Reduced penalty since early game positioning handles this better
        
        return 0.0
    
    def _calculate_advanced_positioning_bonus(self, game, color: DieColor, number: int) -> float:
        """
    ...
    def should_make_move_in_stage(self, game, stage: int) -> bool:
        ...
        # This is the key change: be willing to skip bad moves
        skip_threshold = self._get_skip_threshold(penalty_count, is_active_player, stage)

        if best_score < skip_threshold:
            self.logger.debug(f"{self.name} skipping - best score {best_score:.1f} below threshold {skip_threshold:.1f}")
            log_game_event(
                "AI_STAGE_DECISION",
        ...
        self.logger.debug(f"{self.name} stage {stage} decision: {'participate' if decision else 'skip'} "
    ...
    def _get_skip_threshold(self, penalty_count: int, is_active_player: bool, stage: int) -> float:
        """
    ...
    def get_available_moves(self, game) -> List[Tuple[DieColor, int]]:
        ...
        for color in DieColor:
            ...
            if game_state == GameState.STAGE_1_MOVES:
                ...
                if (self.get_scoresheet().can_mark_number(color, white_sum) and
                    self.can_use_white_sum()):
                    available_moves.append((color, white_sum))
            
            # Stage 2: White + colored combination moves (only for active player)
            elif game_state == GameState.STAGE_2_MOVES and self == game.get_current_player():
                ...
                if color in white_colored_sums:
                    for sum_value in white_colored_sums[color]:
                        if (self.get_scoresheet().can_mark_number(color, sum_value) and 
                            self.can_use_colored_combination()):
